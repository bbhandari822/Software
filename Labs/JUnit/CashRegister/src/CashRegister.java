...
//        if(total_price != pitaVar.applyTaxToPurchase(price)) total_price = 500;
    ...
    public double finalizePurchase(BillPack bp, CoinPack cp) {
        ...
        */
            ...
            System.out.println("Do you need extra money? If so enter 'Yes'");
            Scanner keyboard = new Scanner(System.in);
            String res = keyboard.nextLine();
            if(res.equals("Yes")){

                String[] arr = {"Pennies", "Nickel", "Dime", "Quarter", "One", "Five", "Ten", "Twenty", "Fifty", "Hundred"};
                for(int i = 0; i < arr.length; i++){
                    System.out.println(arr[i]);
                }
                System.out.println("What do you wanna add? ");
                String moneyWantToAdd = keyboard.nextLine();
                int num = 0;
                for(int i=0 ; i< arr.length; i++){
                    if(moneyWantToAdd.equals(arr[i])) {
                    num = i;
                    }
                }
                System.out.println("How many " + moneyWantToAdd + "'s do you want to add");

                int n = Integer.parseInt(keyboard.nextLine());
                switch (num){
                    case 0://penny
                        System.out.println("How many penny do you want to add? ");
                        cp.pennies(n);
                    case 1://nickle
                        System.out.println("How many nickle do you want to add? ");
                        cp.nickles(n);

                    case 2://dime
                        System.out.println("How many dime do you want to add? ");
                        cp.dimes(n);

                    case 3://quarter
                        System.out.println("How many quarter do you want to add? ");
                        cp.quarters(n);
                    ...
                    case 8:
                        System.out.println("How many Fifties do you want to add? ");
                        bp.fifties(n);

                    case 9:
                        System.out.println("How many Hundred do you want to add? ");
                        bp.hundreds(n);
                        break;

                }
            return finalizePurchase(bp, cp);
